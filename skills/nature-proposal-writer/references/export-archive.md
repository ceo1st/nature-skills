...
**.docx 必须用 python-docx 脚本生成，禁止用 pandoc**。pandoc 输出的 .docx 格式粗糙（标题蓝色、无中文字体、无段落格式），不可直接交付。使用 `scripts/build_proposal_docx.py`，该脚本输出格式标准：Times New Roman 12pt + 宋体，Heading 黑体或宋体加粗黑色，1.5 倍行距，四边 1 英寸页边距，表格 10pt 带网格。详细格式规范见 `references/docx-format-standard.md`。Load `pdf` only when PDF is requested.
...
Before writing to vault, ask for explicit approval:

```text
是否归档到 /vault/raw/氯盐储能/研究计划/<project-slug>/ ?
```

Never write `/vault/wiki` as part of the default pipeline.
